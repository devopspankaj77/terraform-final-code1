...
output "vm_public_ips" {
  description = "Map of Windows VM public IPs (only where create_public_ip = true)."
  value       = { for k, v in azurerm_public_ip.pip : k => v.ip_address }
}

output "principal_ids" {
  description = "Map of Windows VM system-assigned managed identity principal IDs."
  value       = { for k, v in azurerm_windows_virtual_machine.vm : k => try(v.identity[0].principal_id, null) }
}
